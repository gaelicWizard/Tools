...
catmovies -auto-chapters -force-same-tracks "${INCLUDE_TRACKS[@]}" "${SKIP_CHAPTERS[@]}" -o "Bleach: Volume 1".mov \
    /Volumes/DroboMedia/iTunes\ Media/TV\ Shows/Bleach/Season\ 1/1-*.m4v \
    "$@"
catmovies -auto-chapters -force-same-tracks "${INCLUDE_TRACKS[@]}" "${SKIP_CHAPTERS[@]}" -o "Bleach: Volume 2".mov \
    /Volumes/DroboMedia/iTunes\ Media/TV\ Shows/Bleach/Season\ 1/2-*.m4v \
    "$@"

catmovies -auto-chapters -force-same-tracks "${INCLUDE_TRACKS[@]}" "${SKIP_CHAPTERS[@]}" -o "Bleach: Volume 12".mov \
    ...
    "$@"
catmovies -auto-chapters -force-same-tracks "${INCLUDE_TRACKS[@]}" "${SKIP_CHAPTERS[@]}" -o "Bleach: Volume 13".mov \
